...
public:
	Quit(Log* logger, PATHS* paths, Config* conf,  bool* running) : Command(logger, paths, conf), running(running) {};
	void run(std::map<PA, std::vector<std::string>>& pargs, std::vector<OA>& oaflags, std::map<OA, std::vector<OA>>& oaoargs, std::map<OA, std::vector<std::string>>& oastrargs);
private:
	bool* running;
};
...
public:
	Show_todos(Log* logger, PATHS* paths, Config* conf) : Command(logger, paths, conf) {};
	void run(std::map<PA, std::vector<std::string>>& pargs, std::vector<OA>& oaflags, std::map<OA, std::vector<OA>>& oaoargs, std::map<OA, std::vector<std::string>>& oastrargs);
};
...
public:
	Activate_mode_command(Log* logger, PATHS* paths, Config* conf, std::unordered_map<int, std::string>* mode_names, int* active_mode, std::vector<std::string>* mode_tags, const std::string* file_ending) : Command(logger, paths, conf), mode_names(mode_names), active_mode(active_mode), mode_tags(mode_tags), file_ending(file_ending) {};
	void run(std::map<PA, std::vector<std::string>>& pargs, std::vector<OA>& oaflags, std::map<OA, std::vector<OA>>& oaoargs, std::map<OA, std::vector<std::string>>& oastrargs);
private:
	std::unordered_map<int, std::string>* mode_names;
	int* active_mode;
...
public:
	Deactivate_mode(Log* logger, PATHS* paths, Config* conf, int* active_mode, std::vector<std::string>* mode_tags) : Command(logger, paths, conf), active_mode(active_mode), mode_tags(mode_tags) {};
	void run(std::map<PA, std::vector<std::string>>& pargs, std::vector<OA>& oaflags, std::map<OA, std::vector<OA>>& oaoargs, std::map<OA, std::vector<std::string>>& oastrargs);
private:
	int* active_mode;
	std::vector<std::string>* mode_tags;
...
public:
	Edit_mode(Log* logger, PATHS* paths, Config* conf, std::unordered_map<int, std::string>* mode_names, std::vector<std::string>* mode_tags, int* active_mode, const std::string* file_ending) : Command(logger, paths, conf), mode_names(mode_names), active_mode(active_mode), mode_tags(mode_tags), file_ending(file_ending) {};
	void run(std::map<PA, std::vector<std::string>>& pargs, std::vector<OA>& oaflags, std::map<OA, std::vector<OA>>& oaoargs, std::map<OA, std::vector<std::string>>& oastrargs);
private:
	std::unordered_map<int, std::string>* mode_names;
	int* active_mode;
...
public:
	Delete_mode(Log* logger, PATHS* paths, Config* conf, int* num_modes, std::unordered_map<int, std::string>* mode_names, std::vector<std::string>* mode_tags, int* active_mode) : Command(logger, paths, conf), num_modes(num_modes), mode_names(mode_names), active_mode(active_mode), mode_tags(mode_tags) {};
	void run(std::map<PA, std::vector<std::string>>& pargs, std::vector<OA>& oaflags, std::map<OA, std::vector<OA>>& oaoargs, std::map<OA, std::vector<std::string>>& oastrargs);
private:
	int* num_modes;
	std::unordered_map<int, std::string>* mode_names;
...
public:
	Create_mode(Log* logger, PATHS* paths, Config* conf, int* num_modes, std::unordered_map<int, std::string>* mode_names, std::vector<std::string>* mode_tags, int* active_mode, const std::string* file_ending) : Command(logger, paths, conf), num_modes(num_modes), mode_names(mode_names), active_mode(active_mode), mode_tags(mode_tags), file_ending(file_ending) {};
	void run(std::map<PA, std::vector<std::string>>& pargs, std::vector<OA>& oaflags, std::map<OA, std::vector<OA>>& oaoargs, std::map<OA, std::vector<std::string>>& oastrargs);
private:
	int* num_modes;
	std::unordered_map<int, std::string>* mode_names;
...
public:
	Open_selection(Log* logger, PATHS* paths, Config* conf, std::vector<std::string>* filter_selection) : Command(logger, paths, conf), filter_selection(filter_selection) {};
	void run(std::map<PA, std::vector<std::string>>& pargs, std::vector<OA>& oaflags, std::map<OA, std::vector<OA>>& oaoargs, std::map<OA, std::vector<std::string>>& oastrargs);
private:
	std::vector<std::string>* filter_selection;
};
...
public:
	Show_filtered_notes(Log* logger, PATHS* paths, Config* conf, int* active_mode, std::string* tmp_filename, std::vector<std::string>* filter_selection, bool* has_pandoc) : Command(logger, paths, conf), active_mode(active_mode), tmp_filename(tmp_filename), has_pandoc(has_pandoc), filter_selection(filter_selection) {};
	void run(std::map<PA, std::vector<std::string>>& pargs, std::vector<OA>& oaflags, std::map<OA, std::vector<OA>>& oaoargs, std::map<OA, std::vector<std::string>>& oastrargs);
private:
	int* active_mode;
